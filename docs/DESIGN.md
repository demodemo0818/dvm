...
- 自前コントロール(素の React。外部プレイヤーライブラリ依存なし): シークバー(バッファ表示付き)・再生/停止・時刻・再生速度(0.5〜2x)・音量/ミュート(localStorage に記憶)・フルスクリーン(Web Fullscreen API)・連続再生トグル(v1.12)・表示サイズトグル(v1.12、mpv のみ)
- ショートカット: Space/K=再生⇄停止、←→=±10 秒、↑↓=音量、M=ミュート、F=フルスクリーン、< >=速度、
  N/P=次/前の動画(v1.8)、T=現在位置をサムネイルに(v1.8)、A=連続再生の切替(v1.12)、
  U=表示サイズ 等倍⇄フィット(v1.12、mpv のみ)、Esc=閉じる(フルスクリーン中は解除のみ)
...
- **ツリーの開閉三角は `▼` 1 種類**にして、閉じているときだけ CSS で `rotate(-90deg)` する。
  `▶` と `▼` は別グリフでフォントによって字幅・線の太さが変わり、開閉で大きさが違って見えたため。
  当たり判定も 20×20px に広げた(`.tree-toggle`)
- **詳細ペインの固定表示**: `inspectorPinned`(ツールバーの 📋 / 設定 `inspector_pinned`)。
...
- **「この位置をサムネイルにする」を下のコントロールバーへ移動**。
  上のバーで `✕ 閉じる` の隣にあり誤爆していた。`PlayerControls` の
  optional prop `onSetThumbnail` にしたので、mpv / WebView2 の 2 か所にあった重複も消えた。
  アイコンは `🖼️`(**U+FE0F 付き**。無いと Windows で細い白黒グリフに落ちて何のボタンか分からない)
