...
**「リリースする」と決めたときだけ**この手順に入ります。機能を足しただけ、直しただけの
コミットではバージョンを上げません。普段の開発は次のようになります。

| やること | バージョン | タグ | Actions |
|---|---|---|---|
| 普段のコミット・push | **触らない** | 打たない | 走らない |
| リリース | 上げる | `v*` を打つ | 走る → Release の下書き |

タグを push しない限り Actions は動かないので、**master に何度 push してもリリースは
起きません**。溜まった変更をまとめて 1 つの版として出す形になります。

### バージョン番号の決め方

これまでは機能追加のたびに minor を上げてきました(v1.42 → v1.43)。公開後もその方針で、
**機能が増えたら minor、不具合修正だけなら patch** を上げます(v1.43.0 → v1.43.1)。
