...
- **設定モーダルは Escape でもオーバーレイのクリックでも閉じる**(v1.38)。
  ただし **v1.38 から設定は「その場保存」で、「保存」ボタンは無い** ——
  チェックを 1 個触っただけでユーザーの設定が変わる。検証で値をいじるなら
  `get_setting` で元の値を控えて、終わったら `set_setting` で必ず戻すこと
  (Tauri のコマンドは `window.__TAURI_INTERNALS__.invoke` で `page.evaluate` から直接叩ける。
  UI の表示ではなく DB の実値を見られるので、保存の確認にはこちらを使う)
- 設定モーダルは**左レール 7 カテゴリの 2 ペイン**。`.settings-nav-item` をクリックして
  切り替え、中身は `.settings-body` に出る。狭い窓では歯車がオーバーフロー
  (`[title="入りきらない操作"]`)に畳まれているので、そこから開く
...
- **閉じないときはまず画面を見ること(v1.40)。** 保存していない再生キューが残っていると、
  アプリが `CloseRequested` を止めて「名前を付けて保存しますか?」を出す。
  `stop.ps1` はそのダイアログを待ち続けるので、ハングと区別が付かない。
  キューが空か保存済みなら尋ねないので、ふだんの検証では起きない
  (判定は `src/lib/queue.ts` の `needsSavePrompt`)
